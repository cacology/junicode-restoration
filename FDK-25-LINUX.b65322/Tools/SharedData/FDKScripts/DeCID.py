...
class ToolPaths:
	def __init__(self):
		...
		if not os.path.exists(self.exe_dir ):
			...
			logMsg("Please re-install teh FDK. Quitting.")
			raise FDKEnvironmentError

		toolList = ["tx", "mergeFonts", "spot"]
		missingTools = []
...
def extractRowFont(toolPaths, srcFontPath, fontPath, PSName, glyphList):
	...
	command = "%s -t1 -decid -std -g %s \"%s\"  \"%s\"  2>&1" % (toolPaths.tx, glyohListArg, srcFontPath, cidFontPath)
	report = FDKUtils.runShellCmd(command)
	if  ("fatal" in report) or ("error" in report):
		if "multiple" in report:			
			...
			command = "%s -dump -4  -g %s \"%s\"  2>&1" % (toolPaths.tx, glyohListArg, srcFontPath)
			report = FDKUtils.runShellCmd(command)
			...
			for fd in fdKeys[1:]:
				...
				glyohListArg = "/" + ",/".join(cidList2)
				cidFontPath = fontPath + ".cid" # A name-keyed font, with the names in the form "cidXXXX".
				command = "%s -t1 -decid -std -g %s \"%s\"  \"%s\"  2>&1" % (toolPaths.tx, glyohListArg, srcFontPath, cidTempFontPath)
				report = FDKUtils.runShellCmd(command)
				if  ("fatal" in report) or ("error" in report):
					logMsg("Error in using tx program to copy glyph subset to row font '%s'." % (fontPath))
					logMsg("Quitting.")
				...
				command = "%s  -std \"%s\" \"%s\" \"%s\" 2>&1" % (toolPaths.mergeFonts, cidTempFontPath2, cidFontPath, cidTempFontPath)
				report = FDKUtils.runShellCmd(command)
				if  ("fatal" in report) or ("error" in report):
					logMsg(report)
	...
	if isNotDefFont:
		...
		# font, as mergeFonts will not copy the same glyph twice.
		tempNotDef = "font.notdef.ps"
		fontDir = os.path.dirname(fontPath)
		if fontDir:
			tempNotDef = os.path.join(fontDir, tempNotDef)
		command = "%s -t1 -std -g 0 \"%s\"  \"%s\"  2>&1" % (toolPaths.tx, cidFontPath, tempNotDef)
		report = FDKUtils.runShellCmd(command)
		if  ("fatal" in report) or ("error" in report):
			logMsg(report)
			logMsg("Error in using tx program to make a temp font for the NotDef font face. '%s'." % (fontPath))
			raise DeCIDError
		command = "%s  -std \"%s\" \"%s\" \"%s\" \"%s\"  2>&1" % (toolPaths.mergeFonts, fontPath, tempNotDef, gaPath, cidFontPath)
	else:
		command = "%s  -std \"%s\" \"%s\" \"%s\"  2>&1" % (toolPaths.mergeFonts, fontPath, gaPath, cidFontPath)
	report = FDKUtils.runShellCmd(command)
	if  ("fatal" in report) or ("error" in report):
		logMsg(report)
		logMsg("Error in using mergeFonts program to rename glyph subset for row font '%s'." % (fontPath))
