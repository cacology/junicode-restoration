...
class table__g_v_a_r(DefaultTable.DefaultTable):
	...
	def compileSharedCoords_(self, axisTags):
		...
		for variations in self.variations.values():
			...
			if self.has_CFF:
				# there is only one glyph variation field, used as a place holder for the shared coordinates.
				break

		if self.has_CFF:
			sharedCoords = [(count, coord) for (coord, count) in coordCount.items()]
		else:
