...
__doc__ = """
MakeCIDFontCube v1.5 Sep 17 2015
MakeCIDFontCube  [ -o <output file path>] [-fh] [-nh] [-ns] [-frf] [-nhr] [-f <path to cidfontinfo file>] [-s <subset file path>]   [-l <path to layout file>]   [-lg <path to LanguageGroup file>] [-vf <path to rotated vertical glyph definition file>]
MakeCIDFontCube  [-u] [-h]
...
-cube	 build a Cube cid font rather than a regular font. Also turns off hinting.
...
kStdFontName = "font.pfa"
kLEFontName = "font.pfa.le"
kRowFontNames = [kLEFontName, kStdFontName]
...
def makeSrcGAFile(toolPaths, srcFontPath, srcGAPath):
	command = "%s -mtx -cubef \"%s\"  2>&1" % (toolPaths.tx, srcFontPath)
	report = FDKUtils.runShellCmd(command)
...
def getFontBBox(txPath, dstPath):
	command = "%s -mtx -cubef \"%s\"  2>&1" % (txPath, dstPath)
	report = FDKUtils.runShellCmd(command)
	if  ("fatal" in report) or ("error" in report):
		logMsg(report)
...
def fontHasLE(fontPath):
	gaCommand = "detype1  \"%s\"  2>&1"  % (fontPath)
	pipe = Popen(gaCommand, shell=True, stdout=PIPE).stdout
	report = pipe.read()
	pipe.close()
	if  ("compose" in report):
		return 1
	else:
		return 0
...
def run(args):
	...
	# Establish font paths. Rebuild flattened font paths, if needed.
	fontPathDict = {}
	isLEFontDict = {}
	logMsg("Checking row fonts for LE's..." + "   " + time.asctime())
	rowFontNameList = kRowFontNames
	for dirName in rowFontDirs:
		for fontName in kRowFontNames:
			fontPath = os.path.join(dirName, fontName)
			if os.path.exists(fontPath):
				break
		hasLE = fontHasLE(fontPath)
		if hasLE:
			cidFonthasLE = hasLE
		isLEFontDict[dirName] = fontHasLE(fontPath)
		fontPathDict[dirName] = fontPath
	...
	for dirName in rowFontDirs:
		...
		# font doesn't exist.	
		if vertFontDict and vertFontDict.has_key(dirName) and (not isLEFontDict[dirName]): 
			# It doesn't exist, but it is a rotated font, so we add it to the list to synthesize.
			toolPaths.vertList.append(dirName)
			fontPath = fontPathDict[fontDir]
	...
	for dirName in toolPaths.vertList: # if is an empty list of vertFontDict is None
		...
		srcPath = fontPathDict[vertEntry[0]]
	...
	if fontinfo.has_key(kHintDictParams):
		...
		for hintDir in hintDirList:
			...
			sourcePath = fontPathDict[sourceDir]
			...
			for fontDir in fontList:
				...
				gaCommand = "%s %s -std -hints \"%s\" \"%s\" \"%s\"  2>&1"  % (toolPaths.mergeFonts, cubeOpt, dstPath, sourcePath, fontPath)
	...
	for dirName in rowFontDirs:
		...
		#
		# If we are not flattening row fonts with LE elements, then we simply don't hint them.
		...
		if (kACPrefix in prefix) or (kGCPrefix in prefix):
			...
			continue
		command = "%s  -cff  -cubef \"%s\"  /dev/null 2>&1" % (toolPaths.tx, fontPath)
		report = FDKUtils.runShellCmd(command)
	...
	for dirName in hintList:
		...
		command = "%s -a -nf  %s \"%s\"   2>&1" % (toolPaths.autohint, noHintSubstitution, fontPath)
		report = FDKUtils.runShellCmd(command)
	...
	for dirName in gcList:
		...
		# convert back to ps
		if os.path.exists(tempPath):
			command = "%s -t1 %s \"%s\"  \"%s\" 2>&1" % (toolPaths.tx, cubeOpt, tempPath, fontPath)
			report = FDKUtils.runShellCmd(command)
			if report:
				print report
			if  "fatal" in report:
	...
	while gaCommandList:
		...
		gaCommand = "%s %s -std -cid \"%s\" \"%s\" %s  %s "  % (toolPaths.mergeFonts, cubeOpt, toolOptions.cidfontinfoPath, dstPath, srcGAPath, srcFontPath)
	...
	if cidFonthasLE:
		cubePath = toolOptions.outFilePath + ".cube"
		shutil.copyfile(dstPath, cubePath)
		if os.path.exists(toolOptions.outFilePath):
			os.remove(toolOptions.outFilePath)
		logMsg(  "Flattening cube font %s to %s." % (cubePath, toolOptions.outFilePath))
		cmd = "tx -t1 -cube  \"%s\" \"%s\" 2>&1" % (cubePath, toolOptions.outFilePath )
		print "Flattening cube", cmd
		report = FDKUtils.runShellCmd(cmd)
		if not os.path.exists(toolOptions.outFilePath):
			logMsg(cmd)
			logMsg(log)
			logMsg("Error: failed to make %s."% ( toolOptions.outFilePath))
	else:
...
def MakeLanguageGroup():
	...
	command = "%s -0  -cubef \"%s\"  2>&1" % ( toolPaths.tx, otfName)
	report = FDKUtils.runShellCmd(command)
	...
	for dirName in rowFontDirs:
		...
		if  not foundPath:
			...
			continue
		command = "%s -6 -cubef \"%s\"  2>&1" % ( toolPaths.tx, fontPath)
		report = FDKUtils.runShellCmd(command)
